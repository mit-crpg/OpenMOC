...
// File: classopenmoc_1_1process_1_1Mesh.xml


%feature("docstring") openmoc::process::Mesh "
...
// File: classopenmoc_1_1options_1_1Options.xml


%feature("docstring") openmoc::options::Options "

Command-line options for runtime configuration of OpenMOC.  

This class parses, interprets and encapsulates the runtime options for OpenMOC.
This class can be instantiate in any OpenMOC Python script which imports the
openmoc.process module. The user may request the value of any or all command
line options and use them as they wish. An example of how the Options class
would be used is given as follows:  

  

C++ includes: openmoc/options.py
";
...
Returns
-------
the number of CUDA thread blocks  
";
...
This method initializes an Options object with default values for each runtime
parameter, and then parses the arguments passed in at runtime and assigns the
appropriate value to each.  
";
...
Returns
-------
the source convergence tolerance  
";
...
// File: classopenmoc_1_1plotter_1_1PlotParams.xml


%feature("docstring") openmoc::plotter::PlotParams "
";
...
This method initializes a PlotParams object with default values for 2D
...
%feature("docstring") PlotParms "

A class to containerize Matplotlib options for 2D spatial plots.  

This class is used to containerize plotting options for the general
plot_spatial_data(...) routine.  

C++ includes: openmoc/plotter.py
";
...
// File: namespaceopenmoc.xml

// File: namespaceopenmoc_1_1compatible.xml
...
Sets the lowest level logging message to print to the screen. This controls the
lowest level for both logging messages in the C++ source code as well as the
user's OpenMOC Python input file. This function would be called at the beginning
...
This method is a wrapper to the log_printf C++ routine. It allows for formatted
messages to be printed to the screen in a similar fashion to the C/C++ printf
method, but with additional formatting provided by the OpenMOC logging
utilities. An example of how this might be used in a OpenMOC Python script is as
follows:  
...
The routine instantiates Material objects with multi-group cross section data
and returns a dictionary of each Material object keyed by its ID. An OpenMOC
Geometry may optionally be given and the routine will directly insert the multi-
group cross sections into each Material in the Geometry. If a Geometry is passed
in, Material objects from the Geometry will be used in place of those
instantiated by this routine. An optional parameter for the domain types may be
used to define whether multi-group cross sections are tabulated by material or
cell in the HDF5 binary file.  
...
Returns
-------
a dictionary of Material objects keyed by ID  
";
...
This routine coputes SuPerHomogenisation (SPH) factors for an an OpenMC MGXS
...
Hebert, A., \"A Consistent Technique for the Pin-by-Pin
          Homogenization of a Pressurized Water Reactor Assembly.\" Nuclear
Science and Engineering, 113 (3), pp. 227-233, 1993.  

The SPH factors are needed to preserve reaction rates in heterogeneous
geometries. The energy condensation process leads to a bias between ultrafine
and coarse energy group calculations. This bias is a result of the use of scalar
flux-weighting to compute MGXS without properly accounting for angular-
...
Returns
-------
a NumPy array of SPH factors and a new openmc.mgxs.Library object with the SPH
factors applied to each MGXS object  
";

// File: namespaceopenmoc_1_1options.xml
...
This method requires that tracks have been generated by a TrackGenerator object.
Each segment is colored by the ID of the unique flat source region it is within.
...
The geometry object must be initialized with Materials, Cells, Universes and
Lattices before being passed into this method. A user may invoke this function
from an OpenMOC Python file as follows:  
...
The Solver must have converged the flat source sources prior to calling this
routine. A user may invoke this function from an OpenMOC Python file as follows:  
...
The IRAMSolver must have computed the eigenmodes prior to calling this routine.
A user may invoke this function from an OpenMOC Python file as follows:  
...
This method requires that Tracks have been generated by a TrackGenerator object
owned by the solver. A user may invoke this function from an OpenMOC Python file
as follows:  
...
The Geometry object must be initialized with Materials, Cells, Universes and
Lattices before being passed into this method. Plotting the CMFD cells requires
that segments must have been created for the geometry and FSR IDs assigned to
regions. A user may invoke this function from an OpenMOC Python file as follows:  
...
energy for one or more flat source regions.  

The Solver must have converged the flat source sources prior to calling this
routine. The routine will generate a step plot of the flat flux across each
energy group.  

An optional parameter for the energy group bounds may be input. The group bounds
should be input in increasing order of energy. If group bounds are not
specified, the routine will use equal width steps for each energy group.  
...
The FSR centroids are plotted as black circles on top of the FSRs if the
centroids boolean is set to True.  
...
The second PlotParams parameter to this routine must include the domain type
encoded in the domains_to_data parameter. In the case of 'material' and 'cell'
domain types, the domains-to-data map must be a Python dictionary with keys
representing each Material/Cell ID and values indicating the data to plot. In
the case of the 'fsr' domain type, the domains-to-data map may be a dictionary,
NumPy array or a Pandas DataFrame with indices representing FSR IDs and values
...
Returns
-------
a list of Matplotlib figures, if requested  
";
...
The method may be used to store the type of Solver used, floating point
precision, exponential evaluation method, number of FSRs, number of materials,
number of energy groups, number of azimuthal angles, number of polar angles,
track spacing, number of Tracks, number of Track segments, number of source
iterations, source convergence tolerance, converged $ k_{eff} $, total runtime,
and number of OpenMP or CUDA threads. In addition, the routine can store the FSR
flux array, FSR source array, and pin and assembly fission rates.  

The routine may export the simulation data to either an HDF5 or a Python pickle
binary file. Users may tell the routine to either create a new binary output
file, or append to an existing file using a timestamp to record multiple
simulation states to the same file.  
...
This routine builds a 2D NumPy array indexed by FSR and energy group for the
corresponding scalar fluxes. The fluxes are organized in the array in order of
increasing FSR and enery group if 'all' FSRs or energy groups are requested (the
default). If the user requests fluxes for specific FSRs or energy groups, then
the fluxes are returned in the order in which the FSRs and groups are enumerated
in the associated paramters.  
...
The fission rates are then exported to a binary HDF5 or python pickle file.  

This routine is intended to be called by the user in Python to compute fission
rates. Typically, the fission rates will represent pin powers. The routine
either exports fission rates to an HDF5 binary file or pickle file with each
fission rate being indexed by a string representing the universe/lattice
hierarchy. This routine may be called from a Python script as follows:  
...
The routine may import the simulation state from either an HDF5 or a Python
pickle binary file created by the store_simulation_state(...) method. The method
may be used to restore the type of Solver used, floating point precision,
exponential evaluation method, number of FSRs, number of materials, number of
energy groups, number of azimuthal angles, number of polar angles, track
spacing, number of Tracks, number of Track segments, number of source
iterations, source convergence tolerance, converged $ k_{eff} $, total runtime,
and number of OpenMP or CUDA threads. In addition, the routine can restore the
FSR flux array, FSR source array.  

Note: If the fission rates were stored in a hdf5 binary file, they are not
restored and returned in this method.  
...
Returns
-------
a Python dictionary of key/value pairs for simulation state data  
";
...
// File: opencg__compatible_8py.xml

// File: log_8py.xml

// File: materialize_8py.xml

// File: options_8py.xml

// File: plotter_8py.xml

// File: process_8py.xml
...
// File: dir_f538075c697d2b17d9e8434deb009c55.xml

// File: dir_e42498f83ad2e028b83ea18abff69fa1.xml

// File: dir_feeaf68b943d4425f333311e6b81e54f.xml
