...
#        Cells and source regions, and fluxes and pin powers.
...
#          unique source region it is within. A user may invoke
...
##
# @brief This method takes in a Geometry object and plots a color-coded 2D
#        surface plot representing the source regions in the Geometry.
...
def plot_source_regions(geometry, gridsize=250, xlim=None, ylim=None,
    ...
    if not isinstance(centroids, bool):
        py_printf('ERROR', 'Unable to plot the source regions since ' +
...
#        surface plot representing the source region scalar fluxes.
# @details The Solver must have converged the source sources prior to
...
#        flux vs. energy for one or more source regions.
# @details The Solver must have converged the source sources prior to
#          calling this routine. The routine will generate a step plot of the
#          flux across each energy group.
#
...
#
# @param solver a Solver object that has converged the source for the Geometry
# @param srs the source region IDs of interest
