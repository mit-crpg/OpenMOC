...
def plot_segments(track_generator, get_figure=False):
    ...
    This method requires that tracks have been generated by a TrackGenerator.
    Each segment is colored by the ID of the unique SR it is within.
...
def plot_source_regions(geometry, gridsize=250, xlim=None, ylim=None,
                        centroids=False, marker_type='o', marker_size=2,
                        get_figure=False, library='matplotlib'):
    """Plots a color-coded 2D surface plot of the SRs in the geometry.
    ...
    ylim : 2-tuple of Real, optional
        The minimim/maximum y-coordinates
    centroids : bool
        Whether to plot the SR centroids (False by default)
    marker_type : str
        The marker type to use for SR centroids ('o' by default)
    marker_size : Integral
        The marker size to use for SR centroids (2 by default)
    get_figure : bool
        Whether to return the Matplotlib figure (only if library='matplotlib')
    library : {'matplotlib', 'pil'}
        The plotting library to use
    ...
    plot_params.filename = 'source-regions-z-{0}'.format(zcoord)
    plot_params.interpolation = 'nearest'
    plot_params.vmin = 0
    ...
    # Plot centroids on top of 2D source region color map
    if centroids:
...
def plot_cmfd_cells(geometry, cmfd, gridsize=250, xlim=None, ylim=None,
    ...
    requires that segments must have been created for the geometry and SR IDs
    assigned to regions.
...
def plot_spatial_fluxes(solver, energy_groups=[1], norm=False, gridsize=250,
                        ...
                        library='matplotlib'):
    """Plot a color-coded 2D surface plot of the SR scalar fluxes for one or
    more energy groups.

    The solver must have converged the SR sources before calling this routine.
...
def plot_energy_fluxes(solver, srs, group_bounds=None, norm=True,
    ...
    """Plot the scalar flux vs. energy for one or more SRs.
    ...
    solver : openmoc.Solver
        An OpenMOC solver used to compute the flux
    srs : Iterable of Integral
        The SRs for which to plot the flux
    ...
    cv.check_type('solver', solver, openmoc.Solver)
    cv.check_type('srs', srs, Iterable, Integral)
...
def plot_fission_rates(solver, norm=False, transparent_zeros=True, gridsize=250,
                       xlim=None, ylim=None, get_figure=False,
                       library='matplotlib'):
    """Plot a color-coded 2D surface plot representing the SR fission rates.

    The Solver must have converged the SR sources before calling this routine.
    The fission rates are computed as the volume-averaged and energy-integrated
    fission rates in each SR.
    ...
    transparent_zeros : bool
        Whether to make all non-fissionable SRs transparent (True by default)
    gridsize : Integral, optional
        The number of grid cells for the plot (250 by default)
    xlim : 2-tuple of Real, optional
        The minimim/maximum x-coordinates
...
def plot_eigenmode_fluxes(iramsolver, eigenmodes=[], energy_groups=[1],
                          ...
                          get_figure=False, library='matplotlib'):
    """Plot the color-coded 2D surface plot of SR scalar fluxes for one or
...
def plot_spatial_data(domains_to_data, plot_params, get_figure=False):
    ...
    domains_to_data : dict or numpy.ndarray or pandas.DataFrame
        ...
        values of the data to plot. In the case of the 'sr' domain type,
        domains_to_data may be a dictionary, NumPy array or a Pandas DataFrame
        indexed by SR ID and values of the data to plot.
...
class PlotParams(object):
    ...
    def domain_type(self, domain_type):
        cv.check_value('domain_type', domain_type, ('material', 'cell', 'sr'))
