 ...
 * @param universes 
 */
Lattice::Lattice(const short int id, const short int num_x,
		short int num_y, double width_x, double width_y,
		short int universes_count, short int *universes): Universe(id) {

	/* Set origin to lower left corner with (x=0, y=0) in center of geometry */
	...
	/* Number of lattice cells along each dimension */
	_num_y = num_y;
	_num_x = num_x;
...
/**
 * Return the id of a flat source region base index (smallest FSR region id
 * within a specific lattice cell)
...
Cell* Lattice::findNextLatticeCell(LocalCoords* coords, double angle,
	...
	short int new_lattice_x;	/* New x lattice cell index */
	short int new_lattice_y;	/* New y lattice cell index */
...
/**
 * Computes the flat source region base indices for each of the lattice cells
 * within this Lattice (ie, the minimum id for the flat source regions within
 * each lattice cell). Returns the number of FSRs in the lattice.
