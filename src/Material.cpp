 ...
 */
NEW_FP_PRECISION* Material::getSigmaT() {
  if (_sigma_t == NULL)
 ...
 */
NEW_FP_PRECISION* Material::getSigmaS() {
  if (_sigma_s == NULL)
 ...
 */
NEW_FP_PRECISION* Material::getSigmaF() {
  if (_sigma_f == NULL)
 ...
 *         multiplied by nu \f$ \nu \f$
 */
NEW_FP_PRECISION* Material::getNuSigmaF() {
  if (_nu_sigma_f == NULL)
 ...
 */
NEW_FP_PRECISION* Material::getChi() {
  if (_chi == NULL)
 ...
 */
NEW_FP_PRECISION* Material::getFissionMatrix() {
  if (_fiss_matrix == NULL)
 ...
 */
NEW_FP_PRECISION Material::getSigmaTByGroup(int group) {
  if (_sigma_t == NULL)
    log_printf(ERROR, "Unable to return Material %d's total "
               "cross section since it has not yet been set", _id);
 ...
 */
NEW_FP_PRECISION Material::getSigmaSByGroup(int origin, int destination) {
  if (_sigma_s == NULL)
    log_printf(ERROR, "Unable to return Material %d's scattering "
               "cross section since it has not yet been set", _id);
 ...
 */
NEW_FP_PRECISION Material::getSigmaFByGroup(int group) {
  if (_sigma_f == NULL)
    log_printf(ERROR, "Unable to return material %d's fission "
               "cross section since it has not yet been set", _id);
 ...
 */
NEW_FP_PRECISION Material::getNuSigmaFByGroup(int group) {
  if (_nu_sigma_f == NULL)
 ...
 */
NEW_FP_PRECISION Material::getChiByGroup(int group) {
  if (_chi == NULL)
    log_printf(ERROR, "Unable to return Material %d's chi spectrum "
               "since it has not yet been set", _id);
 ...
 */
NEW_FP_PRECISION Material::getFissionMatrixByGroup(int origin, int destination) {
  if (_fiss_matrix == NULL)
    log_printf(ERROR, "Unable to return Material %d's fission matrix "
...
void Material::setNumEnergyGroups(const int num_groups) {
  ...
  _sigma_t = new NEW_FP_PRECISION[_num_groups];
  _sigma_f = new NEW_FP_PRECISION[_num_groups];
  _nu_sigma_f = new NEW_FP_PRECISION[_num_groups];
  _chi = new NEW_FP_PRECISION[_num_groups];
  _sigma_s = new NEW_FP_PRECISION[_num_groups*_num_groups];

  /* Assign the null vector to each data array */
  memset(_sigma_t, 0.0, sizeof(NEW_FP_PRECISION) * _num_groups);
  memset(_sigma_f, 0.0, sizeof(NEW_FP_PRECISION) * _num_groups);
  memset(_nu_sigma_f, 0.0, sizeof(NEW_FP_PRECISION) * _num_groups);
  memset(_chi, 0.0, sizeof(NEW_FP_PRECISION) * _num_groups);
  memset(_sigma_s, 0.0, sizeof(NEW_FP_PRECISION) * _num_groups * _num_groups);
}
...
void Material::setSigmaT(double* xs, int num_groups) {
  ...
  for (int i=0; i < _num_groups; i++) {
    ...
    if (fabs(xs[i]) < ZERO_SIGMA_T) {
      ...
      _sigma_t[i] = NEW_FP_PRECISION(ZERO_SIGMA_T);
    }
    else
      _sigma_t[i] = NEW_FP_PRECISION(xs[i]);
  }
}
...
void Material::setSigmaTByGroup(double xs, int group) {
  ...
  if (group <= 0 || group > _num_groups)
    ...
    if (fabs(xs) < ZERO_SIGMA_T) {
      ...
      _sigma_t[group-1] = NEW_FP_PRECISION(ZERO_SIGMA_T);
    }
    else
      _sigma_t[group-1] = NEW_FP_PRECISION(xs);
}
...
void Material::buildFissionMatrix() {
  ...
  _fiss_matrix = new NEW_FP_PRECISION[_num_groups*_num_groups];
...
void Material::alignData() {
  ...
  int size = _num_vector_groups * VEC_LENGTH * sizeof(NEW_FP_PRECISION);

  /* Allocate word-aligned memory for cross-section data arrays */
  NEW_FP_PRECISION* new_sigma_t = (NEW_FP_PRECISION*)MM_MALLOC(size, VEC_ALIGNMENT);
  NEW_FP_PRECISION* new_sigma_f = (NEW_FP_PRECISION*)MM_MALLOC(size, VEC_ALIGNMENT);
  NEW_FP_PRECISION* new_nu_sigma_f=(NEW_FP_PRECISION*)MM_MALLOC(size, VEC_ALIGNMENT);
  NEW_FP_PRECISION* new_chi = (NEW_FP_PRECISION*)MM_MALLOC(size, VEC_ALIGNMENT);
  ...
  size *= VEC_LENGTH * sizeof(NEW_FP_PRECISION);
  NEW_FP_PRECISION* new_fiss_matrix = (NEW_FP_PRECISION*)MM_MALLOC(size, VEC_ALIGNMENT);
  NEW_FP_PRECISION* new_sigma_s = (NEW_FP_PRECISION*)MM_MALLOC(size, VEC_ALIGNMENT);
  ...
  size = _num_vector_groups * VEC_LENGTH * sizeof(NEW_FP_PRECISION);
  ...
  size = _num_groups * sizeof(NEW_FP_PRECISION);
...
void Material::transposeProductionMatrices() {
  ...
  if (_fiss_matrix != NULL)
    matrix_transpose<NEW_FP_PRECISION>(_fiss_matrix, num_groups, num_groups);
  if (_sigma_s != NULL)
    matrix_transpose<NEW_FP_PRECISION>(_sigma_s, num_groups, num_groups);
}
