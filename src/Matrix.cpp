...
Matrix::Matrix(omp_lock_t* cell_locks, int num_x, int num_y, int num_z,
  ...
  for (int i=0; i < _num_rows; i++)
    _LIL.push_back(std::map<int, NEW_PRECISION>());
...
void Matrix::incrementValue(int cell_from, int group_from,
                            int cell_to, int group_to, NEW_PRECISION val) {
...
void Matrix::convertToCSR() {
  ...
  _A = new NEW_PRECISION[NNZ];
  _IA = new int[_num_rows+1];
  _JA = new int[NNZ];
  _DIAG = new NEW_PRECISION[_num_rows];
  std::fill_n(_DIAG, _num_rows, 0.0);

  /* Form arrays */
  int j = 0;
  std::map<int, NEW_PRECISION>::iterator iter;
 ...
 */
NEW_PRECISION Matrix::getValue(int cell_from, int group_from,
