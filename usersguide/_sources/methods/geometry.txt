...
OpenMOC uses a technique known as `constructive solid geometry`_ (CSG) to build
...
In OpenMOC, every surface defined by the user is assigned an integer to uniquely
...
The ability to form regions based on bounding quadratic surfaces enables OpenMOC
...
.. table:: Surface types available in OpenMOC.
...
OpenMOC supports universe-based geometry similar to the likes of MCNP_ and
...
Each cell in OpenMOC can either be filled with a normal material or with a
...
use of lattices and universes can be seen in the OpenMOC model for the `Monte
...
OpenMOC, this parameter is set to :math:`\epsilon = 10^{-14}` since all floating
