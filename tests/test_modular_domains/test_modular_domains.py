...
class ModularDomainsTestHarness(TestHarness):
    """An eigenvalue calculation in a 3D lattice with 7-group C5G7 data, cut in 
    2*3*4 modular domains. Modular domains are meant to replicate domain 
    decompositions using fewer nodes."""

    def __init__(self):
        super(ModularDomainsTestHarness, self).__init__()
    ...
    def _get_results(self, num_iters=True, keff=True, fluxes=False,
        ...
        return super(ModularDomainsTestHarness, self)._get_results(
...
if __name__ == '__main__':
    harness = ModularDomainsTestHarness()
    harness.main()
