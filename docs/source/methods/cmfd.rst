...
:math:`g, g \prime`  Energy group index             :math:`x, y`         Position variable
===================  =============================  ===================  =============================
...
The cross sections for the CMFD diffusion equation are generated by energy-condensation and area-averaging of the cross sections from the fine mesh as shown in equation :eq:`eqn=xs-condensation`. The energy group structure of the CMFD diffusion equations does not necessarily have to be the same as the energy group structure used in MOC. For example, :ref:`Figure 2 <figure-coarse-group-CMFD>` shows the various ways to formulate the CMFD group structure to accelerate a four group MOC problem. When the CMFD group structure differs from the MOC groups structure, we rename call it coarse group coarse mesh finite difference (CGCMFD).
...
By condensing the cross sections in this way, we require reaction rates to be conserved.
...
.. math::
   ...
   \varSigma^{A,i,j}_g = \frac{\displaystyle\sum\limits_{r \in (i,j)} \Sigma^{A}_{r,g} \Phi_{r,g} A_r}{\displaystyle\sum\limits_{r \in (i,j)} \Phi_{r,g} A_r}

.. math::

   \varSigma^{S,i,j}_{g \rightarrow g \prime} = \frac{\displaystyle\sum\limits_{r \in (i,j)} \Sigma^S_{r,g \rightarrow g \prime} \Phi_{r,g} A_r}{\displaystyle\sum\limits_{r \in (i,j)} \Phi_{r,g} A_r}

.. math::

   D_g^{i,j} = \frac{\displaystyle\sum\limits_{r \in (i,j)} \frac{1}{3 \Sigma^T_{r,g}} \Phi_{r,g} A_r}{\displaystyle\sum\limits_{r \in (i,j)} \Phi_{r,g} A_r}

.. math::

   \chi_g^{i,j} = \frac{\displaystyle\sum\limits_{r \in (i,j)} \displaystyle\sum\limits_{g \prime = 1}^G \chi_{r,g} \nu \Sigma^T_{r,g \prime} \Phi_{r,g \prime} A_r}{\displaystyle\sum\limits_{r \in (i,j)} \displaystyle\sum\limits_{g \prime = 1}^G \displaystyle\sum\limits_{g \prime \prime = 1}^G \chi_{r,g \prime \prime} \nu \Sigma^{F}_{r,g \prime} \Phi_{r,g \prime} A_r}

.. math::

   \phi_g^{i,j} = \frac{\displaystyle\sum\limits_{r \in (i,j)} \Phi_{r,g} A_r}{\displaystyle\sum\limits_{r \in (i,j)} A_r}

Where :math:`i` and :math:`j` denote the mesh cell indices in the x and y directions, respectively. In our derivation of MOC we have assumed isotropic scattering in the lab system which makes the total cross section, :math:`\Sigma^T`, equal to the transport cross section, :math:`\Sigma^{tr}`. This allows us to approximate the diffusion coefficient on the fine mesh as :math:`\frac{1}{3 \Sigma^T}` instead of its' defined value of :math:`\frac{1}{3 \Sigma^{tr}}`. Approximating the diffusion coefficient in this manner works well for optically thin regions, but induces significant error where the width of a region is much greater than the neutron mean free path (i.e. the region is optically thick). Modifications to the diffusion coefficient for optically thick regions will be discussed in :ref:`Section 7.1 <optically-thick>`.
...
The diffusion operator in \autoref{eqn:dif-eqn} can be expanded to yield:    

.. math::
   :label: dif-eqn-expand-1

    - \frac{\partial}{\partial x} D_g (x,y) \frac{\partial}{\partial x} \phi_g (x,y) - \frac{\partial}{\partial y} D_g (x,y) \frac{\partial}{\partial y} \phi_g (x,y) + \varSigma^A_{g} \phi_g (x,y) + \sum_{\substack{g \prime = 1 \\ g \prime \neq g}}^G \varSigma^S_{s,g \rightarrow g \prime} (x,y) \phi_g (x,y) = \frac{\chi_g (x,y)}{k_{eff}} \sum_{g \prime = 1}^G \nu \varSigma^F_{g \prime} (x,y) \phi_{g \prime} (x,y) + \sum_{\substack{g \prime = 1 \\ g \prime \neq g}}^G \varSigma^S_{g \prime \rightarrow g} (x,y) \phi_{g \prime} (x,y)
...
.. math::
    ...
    - \int_{i-\frac{1}{2}}^{i+\frac{1}{2}} \int_{j-\frac{1}{2}}^{j+\frac{1}{2}} \frac{\partial}{\partial x} D_g^{i,j} \frac{\partial}{\partial x} \phi_g^{i,j} dx dy = - \Delta y^{i,j} (J_g^{i+\frac{1}{2},j} - J_g^{i-\frac{1}{2},j})
    
Where :math:`J_g^{i+\frac{1}{2},j}` and :math:`J_g^{i-\frac{1}{2},j}` represent the surface-averaged net current across the right surface and across the left surface, respectively. An analogous equation can be written for streaming in the y direction. 
...
.. math::
    ...
    J_{g}^{i+\frac{1}{2},j,+} = \left. - D_g^{i+1,j} \frac{d \phi_g^{i+\frac{1}{2},j}}{d x} \right|_{x^{i+\frac{1}{2},j,+}} = - D_g^{i+\frac{1}{2},j} \frac{\phi_g^{i+1,j} - \phi_g^{i+\frac{1}{2},j}}{\frac{\Delta x^{i+1,j}}{2}}\\ \nonumber
    J_{g}^{i+\frac{1}{2},j,-} = \left. - D_g^{i,j} \frac{d \phi_g^{i+\frac{1}{2},j}}{d x} \right|_{x^{i+\frac{1}{2},j,-}} = - D_g^{i,j} \frac{\phi_g^{i+\frac{1}{2},j} - \phi_g^{i,j}}{\frac{\Delta x^{i,j}}{2}}
    
Where :math:`J_{g}^{i+\frac{1}{2},j,+}` and :math:`J_{g}^{i+\frac{1}{2},j,-}` are the surface-averaged partial currents as approximated using a backward difference approximation from the cell at :math`(i,j)` and a forward difference approximation from the cell at :math:`(i+1,j)`, respectively. Equating these representations of the current we can solve for the flux at the surface:

.. math::

    \phi_g^{i+\frac{1}{2},j} = \frac{D_g^{i,j} \phi_g^{i,j} \Delta x^{i+1,j} + D_g^{i+1,j} \phi_g^{i+1,j} \Delta x^{i,j}}{D_g^{i,j}\Delta x^{i+1,j} + D_g^{i+1,j} \Delta x^{i,j}}
...
.. math::
   :label: eqn-alg-net-current
    ...
    J_{g}^{i+\frac{1}{2},j} = - \hat{D}^{i+\frac{1}{2},j} (\phi^{i+1,j} - \phi^{i,j})
...
.. math::
   :label: eqn-surf-dif-coef
    ...
    \hat{D}_g^{i+\frac{1}{2},j} = \frac{2 D_g^{i,j} D_g^{i+1,j}}{D_g^i\Delta x^{i+1,j} + D_g^{i+1,j} \Delta x^{i,j}}
...
.. math::
   :label: eqn-dif-eqn-simple
    ...
    - \Delta y^{i,j} (J_g^{i+\frac{1}{2},j} - J_g^{i-\frac{1}{2},j}) - \Delta x^{i,j} (J_g^{i,j+\frac{1}{2}} - J_g^{i,j-\frac{1}{2}}) + \Delta x^{i,j} \Delta y^{i,j} \varSigma_{g}^{A,i,j} \phi_g^{i,j} + \Delta x^{i,j} \Delta y^{i,j} \sum_{\substack{g \prime = 1 \\ g \prime \neq g}}^G \varSigma_{g \rightarrow g \prime}^{S,i,j} \phi_g^{i,j} = \Delta x^{i,j} \Delta y^{i,j} \frac{\chi_g^{i,j}}{k_{eff}} \sum_{g \prime = 1}^G \nu \varSigma_{g \prime}^{F,i,j} \phi_{g \prime}^{i,j} + \Delta x^{i,j} \Delta y^{i,j} \sum_{\substack{g \prime = 1 \\ g \prime \neq g}}^G \varSigma_{g \prime \rightarrow g}^{S,i,j} \phi_{g \prime}^{i,j} 
...
.. math::
   :label: eqn-surf-avg-tallied-current-2
    ...
    \tilde{J}_g^{i + \frac{1}{2}, j} = \sum_{k \cap (i+\frac{1}{2},j)} \Psi_{k,g,p} \frac{\omega_p \omega_k}{2 \pi \cos \theta_k} \sin \theta_p \cdot \hat{n}
...
.. math::
   :label: eqn-surf-avg-tallied-current-3
    ...
    \tilde{J}_g^{i + \frac{1}{2}, j} = \sum_{k \cap (i+\frac{1}{2},j)} \Psi_{k,g,p} \frac{\omega_p \omega_k}{2 \pi} \sin \theta_p
...
.. math::
   :label: eqn-alg-net-current-cor
    ...
    \frac{\tilde{J}_{g}^{i+\frac{1}{2},j}}{\Delta y^{i,j}} = - \hat{D}^{i+\frac{1}{2},j} (\phi^{i+1,j} - \phi^{i,j}) - \tilde{D}^{i+\frac{1}{2},j} (\phi^{i+1,j} + \phi^{i,j})
    
Where :math:`\tilde{D}` is the nonlinear diffusion coefficient correction factor. Note that current expression computes the \textit{surface-averaged} net current whereas the current tallied from MOC has not been averaged over the surface; therefore, in :eq:`eqn-alg-net-current-cor` the net current tallied from MOC has been divided by the length of the surface that is being crossed. :math:`\tilde{D}` is computed to make :eq:`eqn-alg-net-current-cor` valid for the tallied net surface current in MOC for the most recent transport sweep:

.. math::
   :label: eqn-dif-cor-factor

    \tilde{D}^{i+\frac{1}{2},j} = \frac{- \hat{D}^{i+\frac{1}{2},j} (\phi^{i+1,j} - \phi^{i,j}) - \frac{\tilde{J}_{g}^{i+\frac{1}{2},j}}{\Delta y^{i,j}}}{(\phi^{i+1,j} + \phi^{i,j})}
...
.. figure:: ../../img/corner-crossing.png
   ...
   **Figure 5**: Illustration of track crossing directly through the corner of a cell. The two main approximations for treating currents are denoted \textbf{1} and \textbf{2} and described below.

There are three main approximations to treat corner crossings:


1. Consistently tallying the current from tracks through specific corners to only one of the surfaces. This approximation effectively moves the track far enough to one side such that the entire segment crosses a single surface. 
2. Split the current evenly among the surfaces. This approximation effectively splits the track into two partially-weighted segments and moves them to either side of the corner such that each new partially-weighted track only sweeps across one surface.
3. A hybrid of approximations 1 and 2 where tracks that directly cross through a corner are treated with approximation 2 and tracks the directly cross through one surface but sweep through a corner are treated with approximation 1.

Note that tracks are not physically moved in any of these cases; rather, we make the assumption that they are moved when we tally the surface currents. In OpenMOC we have implemented approximation 3 where only tracks that directly intersect a corner are split among the surfaces. The partial weight given to each new track is half the original weight, regardless of track angle. While applying approximation 2 for all tracks would be more accurate, this would require additional computational costs when the tracks are generated, which can be a signficant portion of computational time for large geometries. The tallies for a track that crosses from cell :math:`(i,j)` to :math:`(i+1,j+1)` for this approximation are given in :eq:`eqn-corner-crossing`.

.. math::
   :label: eqn-corner-crossing

   \tilde{J}_g^{i + \frac{1}{2},j} = \frac{1}{2} \sum_{k \cap (i+\frac{1}{2},j+\frac{1}{2})} \Psi_{k,g,p} \frac{\omega_p \omega_k}{2 \pi} \sin \theta_p \\
   ...
   \tilde{J}_g^{i + \frac{1}{2}, j + 1} = \frac{1}{2} \sum_{k \cap (i+\frac{1}{2},j+\frac{1}{2})} \Psi_{k,g,p} \frac{\omega_p \omega_k}{2 \pi} \sin \theta_p
...
.. math::
   ...
   \alpha_{g,k}^{i,j,x} = \left( \frac{1 + e^{-\frac{\Delta x^{i,j}}{3 D_g^{i,j} \omega_{m(k)}}}}{1 - e^{-\frac{\Delta x^{i,j}}{3 D_g^{i,j} \omega_{m(k)}}}} \right) - \frac{2 \omega_{m(k)}}{\Delta x^{i,j}}
...
:math:`N`               The numbert of coarse mesh cells, I x J
:math:`G`               The numbert of energy groups
...
In :ref:`Section 7.4 <cmfd-accel-moc>` we discuss how this matrix equation will be solved and used to accelerate the solution of the MOC solve.
...
.. math::
   :label: eqn-nldif-damp
    ...
    \tilde{D}^{i+\frac{1}{2},j,(n)}_g = (1 - \omega_d) \tilde{D}^{i+\frac{1}{2},j,(n-1)}_g + \omega_d \frac{- \hat{D}^{i+\frac{1}{2},j,(n)}_g (\phi^{i+1,j,(n)}_g - \phi^{i,j,(n)}_g) - \frac{\tilde{J}_{g}^{i+\frac{1}{2},j,(n)}}{\Delta y^{i,j}}}{(\phi_g^{i+1,j,(n)} + \phi^{i,j,(n)}_g)}
...
.. math::
   ...
   \Phi_{r,g} = \Phi_{r,g} \frac{\phi^{i,j,new}_{g}}{\phi^{i,j,old}_{g}}  \qquad \forall \quad r \in (i,j)

where :math:`\phi^{i,j,old}_{g}` is the coarse mesh scalar flux computed using equation :eq:`xs-condensation` and :math:`\phi^{i,j,new}_{g}` is the converged CMFD coarse mesh scalar flux.
...
.. [8] E. Larsen, "Infinite Medium Solutions to the Transport Equation, $S_n$ Discretization Schemes, and the Diffusion Approximation. " *Proceedings of the Joint International Topical Meeting on Mathematics and Computation and Supercomputing in Nuclear Applications*, Salt Lake City, UT, USA (2001).
